...
main (int argc, char* argv[])
{
  ...
  double perFlowCIR = totalAssuredBps / N_SOURCES;
  ...
  flows.reserve (N_SOURCES);

  Time peakInterval = Seconds ((pktSize * 8.0) / 1e6); // 1 Mb/s peak
  ...
  for (auto& kv : flowMon->GetFlowStats ())
    {
      if (classifier->FindFlow (kv.first).destinationPort == port)
        {
          double bps = kv.second.rxBytes * 8.0 / statsWindow;
          totalBps += bps;
          nFlows++;
        }
    }
  ...
  std::cout << "\n=== RIO Simulation Results ===" << std::endl;
  ...
  std::ofstream out("scratch/rio-wireless_out.csv", std::ios::app);
  out << simCase << ","
      << assuredRate << ","
      << N_SOURCES << ","
      << nFTP << ","
