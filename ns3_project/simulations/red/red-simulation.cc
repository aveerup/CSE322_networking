...
main (int argc, char* argv[])
{
  ...
  // RED/G-RIO-like parameters (paper table for 50ms target delay)
  ...
  // Paper-style marking profile knob for comparability with RIO studies.
  // RED itself ignores packet color/priority, so this does not alter RED drop logic.
  ...
  std::ofstream out("scratch/red_out.csv", std::ios::app);
  out << simCase << "," 
      << assuredRate << "," 
      << nSources << ","
      << nFTP << ","
